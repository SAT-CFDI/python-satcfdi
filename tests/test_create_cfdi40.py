...
@pytest.mark.parametrize('valor_unitario', [Decimal('100.03'), Decimal('100.04'), Decimal('100.05')])
def test_suma_conceptos_iva(valor_unitario):
    rfc = 'xiqb891116qe4'
    signer = get_signer(rfc)
    emisor = cfdi40.Emisor(
            rfc=signer.rfc,
    ...
    invoice.sign(signer)
    verify_invoice(invoice, f"iva_concepto_suma_{valor_unitario}")
    ...
    total = valor_unitario * 6 * Decimal('1.16')
    assert invoice["Total"] <= total.quantize(Decimal('0.01'), rounding=ROUND_CEILING)
    assert invoice["Total"] >= total.quantize(Decimal('0.01'), rounding=ROUND_FLOOR)
