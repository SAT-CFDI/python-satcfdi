{'Addenda': None,
 'Certificado': '',
 'CfdiRelacionados': None,
 'Complemento': {'Pago': [{'DoctoRelacionado': [{'EquivalenciaDR': 1,
                                                 'Folio': '123456',
                                                 'IdDocumento': '6d7434a6-e3f2-47ad-9e4c-08849946afa0',
                                                 'ImpPagado': Decimal('3245.12'),
                                                 'ImpSaldoAnt': Decimal('5000.43'),
                                                 'ImpSaldoInsoluto': Decimal('1755.31'),
                                                 'ImpuestosDR': {'RetencionesDR': [{'BaseDR': Decimal('3403.97'),
                                                                                    'ImporteDR': Decimal('340.40'),
                                                                                    'ImpuestoDR': Code('001', 'ISR'),
                                                                                    'TasaOCuotaDR': Decimal('0.100000'),
                                                                                    'TipoFactorDR': Code('Tasa', None)},
                                                                                   {'BaseDR': Decimal('3403.97'),
                                                                                    'ImporteDR': Decimal('363.09'),
                                                                                    'ImpuestoDR': Code('002', 'IVA'),
                                                                                    'TasaOCuotaDR': Decimal('0.106667'),
                                                                                    'TipoFactorDR': Code('Tasa', None)}],
                                                                 'TrasladosDR': [{'BaseDR': Decimal('3403.97'),
                                                                                  'ImporteDR': Decimal('544.63'),
                                                                                  'ImpuestoDR': Code('002', 'IVA'),
                                                                                  'TasaOCuotaDR': Decimal('0.160000'),
                                                                                  'TipoFactorDR': Code('Tasa', None)}]},
                                                 'MetodoDePagoDR': Code('PPD', 'Pago en parcialidades o diferido'),
                                                 'MonedaDR': Code('MXN', 'Peso Mexicano'),
                                                 'NumParcialidad': 2,
                                                 'ObjetoImpDR': '02',
                                                 'Serie': 'A'}],
                           'FechaPago': datetime.datetime(2020, 1, 2, 22, 40, 38),
                           'FormaDePagoP': '03',
                           'ImpuestosP': {'RetencionesP': [{'ImporteP': Decimal('340.40'),
                                                            'ImpuestoP': Code('001', 'ISR')},
                                                           {'ImporteP': Decimal('363.09'),
                                                            'ImpuestoP': Code('002', 'IVA')}],
                                          'TrasladosP': [{'BaseP': Decimal('3403.97'),
                                                          'ImporteP': Decimal('544.63'),
                                                          'ImpuestoP': Code('002', 'IVA'),
                                                          'TasaOCuotaP': Decimal('0.160000'),
                                                          'TipoFactorP': Code('Tasa', None)}]},
                           'MonedaP': Code('MXN', 'Peso Mexicano'),
                           'Monto': Decimal('3245.12'),
                           'TipoCambioP': 1}],
                 'Totales': {'MontoTotalPagos': Decimal('3245.12'),
                             'TotalRetencionesISR': Decimal('340.40'),
                             'TotalRetencionesIVA': Decimal('363.09'),
                             'TotalTrasladosBaseIVA16': Decimal('3403.97'),
                             'TotalTrasladosImpuestoIVA16': Decimal('544.63')},
                 'Version': '2.0'},
 'Conceptos': [{'ACuentaTerceros': None,
                'Cantidad': 1,
                'ClaveProdServ': '84111506',
                'ClaveUnidad': 'ACT',
                'ComplementoConcepto': None,
                'CuentaPredial': None,
                'Descripcion': 'Pago',
                'Descuento': None,
                'Importe': Decimal('0'),
                'Impuestos': None,
                'InformacionAduanera': None,
                'NoIdentificacion': None,
                'ObjetoImp': '01',
                'Parte': None,
                'Unidad': None,
                'ValorUnitario': Decimal('0'),
                '_traslados_incluidos': False}],
 'CondicionesDePago': None,
 'Confirmacion': None,
 'Descuento': None,
 'Emisor': {'Nombre': 'BERENICE XIMO QUEZADA',
            'RegimenFiscal': '606',
            'Rfc': RFC('XIQB891116QE4')},
 'Exportacion': '01',
 'Fecha': datetime.datetime(2020, 1, 1, 22, 40, 38),
 'Folio': '123456',
 'FormaPago': None,
 'Impuestos': None,
 'InformacionGlobal': None,
 'LugarExpedicion': '56820',
 'MetodoPago': None,
 'Moneda': 'XXX',
 'NoCertificado': '30001000000400002311',
 'Receptor': {'DomicilioFiscalReceptor': '59820',
              'Nombre': 'KIJ, S.A DE C.V.',
              'RegimenFiscalReceptor': Code('601', 'General de Ley Personas Morales'),
              'Rfc': 'KIJ0906199R1',
              'UsoCFDI': 'CP01'},
 'Sello': '',
 'Serie': 'A',
 'SubTotal': Decimal('0'),
 'TipoCambio': None,
 'TipoDeComprobante': 'P',
 'Total': Decimal('0'),
 'Version': '4.0'}