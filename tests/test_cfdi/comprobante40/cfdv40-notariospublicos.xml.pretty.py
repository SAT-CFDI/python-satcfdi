{'Certificado': '',
 'CfdiRelacionados': [{'CfdiRelacionado': ['F4F09AEF-57F2-4BE0-A828-87D1A80ED61C'],
                       'TipoRelacion': Code('09', 'Factura generada por pagos diferidos')}],
 'Complemento': {'NotariosPublicos': {'DatosAdquiriente': {'CoproSocConyugalE': 'No',
                                                           'DatosUnAdquiriente': {'ApellidoMaterno': 'Villarreal',
                                                                                  'ApellidoPaterno': 'Garza',
                                                                                  'CURP': 'LIGJ840811HNLMRS02',
                                                                                  'Nombre': 'Benjamín',
                                                                                  'RFC': 'LIGJ8408112U5'}},
                                      'DatosEnajenante': {'CoproSocConyugalE': 'No',
                                                          'DatosUnEnajenante': {'ApellidoMaterno': 'Guerrero',
                                                                                'ApellidoPaterno': 'Limón',
                                                                                'CURP': 'LIGJ840811HNLMRS02',
                                                                                'Nombre': 'Jesús',
                                                                                'RFC': 'LIGJ8408112U5'}},
                                      'DatosNotario': {'Adscripcion': 'Prueba',
                                                       'CURP': 'LIGJ840811HNLMRS02',
                                                       'EntidadFederativa': Code('19', 'NUEVO LEON'),
                                                       'NumNotaria': 777},
                                      'DatosOperacion': {'FechaInstNotarial': datetime.date(2014, 5, 2),
                                                         'IVA': Decimal('10'),
                                                         'MontoOperacion': Decimal('100'),
                                                         'NumInstrumentoNotarial': 999999,
                                                         'Subtotal': Decimal('110')},
                                      'DescInmuebles': [{'Calle': 'Humboldt',
                                                         'CodigoPostal': '45781',
                                                         'Colonia': 'Mirador',
                                                         'Estado': Code('19', 'NUEVO LEON'),
                                                         'Localidad': 'Monterrey',
                                                         'Municipio': 'Monterrey',
                                                         'NoExterior': '1119',
                                                         'NoInterior': 'D',
                                                         'Pais': 'MEX',
                                                         'Referencia': '2do. '
                                                                       'piso',
                                                         'TipoInmueble': '01'}],
                                      'Version': '1.0'}},
 'Conceptos': [{'Cantidad': Decimal('1'),
                'ClaveProdServ': Code('84111506', 'Servicios de facturación'),
                'ClaveUnidad': Code('ACT', 'Actividad'),
                'Descripcion': 'Descripcion',
                'Importe': Decimal('0'),
                'ObjetoImp': Code('01', 'No objeto de impuesto'),
                'ValorUnitario': Decimal('0')}],
 'Emisor': {'Nombre': 'Esta es una demostración',
            'RegimenFiscal': Code('622', 'Actividades Agrícolas, Ganaderas, Silvícolas y Pesqueras'),
            'Rfc': ' AAA010101AAA'},
 'Exportacion': Code('03', 'Temporal'),
 'Fecha': datetime.datetime(2021, 12, 8, 23, 59, 59),
 'FormaPago': Code('02', 'Cheque nominativo'),
 'LugarExpedicion': '99999',
 'Moneda': Code('XXX', 'Los códigos asignados para las transacciones en que intervenga ninguna moneda'),
 'NoCertificado': '30001000000300023708',
 'Receptor': {'DomicilioFiscalReceptor': '99999',
              'Nombre': 'Juanito Bananas De la Sierra',
              'RegimenFiscalReceptor': Code('630', 'Enajenación de acciones en bolsa de valores'),
              'Rfc': 'BASJ600902KL9',
              'UsoCFDI': Code('P01', 'Por definir')},
 'Sello': '',
 'SubTotal': Decimal('0'),
 'TipoDeComprobante': Code('P', 'Pago'),
 'Total': Decimal('0'),
 'Version': '4.0'}