{'Addenda': {},
 'Cert': '',
 'Complemento': {'Dividendos': {'DividOUtil': {'CveTipDivOUtil': Code('01', 'Proviene de CUFIN'),
                                               'MontDivAcumExt': Decimal('0'),
                                               'MontDivAcumNal': Decimal('5.00'),
                                               'MontISRAcredNal': Decimal('30.00'),
                                               'MontISRAcredRetExtranjero': Decimal('0'),
                                               'MontISRAcredRetMexico': Decimal('35.00'),
                                               'MontRetExtDivExt': Decimal('0'),
                                               'TipoSocDistrDiv': 'Sociedad '
                                                                  'Nacional'},
                                'Remanente': {'ProporcionRem': Decimal('0.00')},
                                'Version': '1.0'},
                 'TimbreFiscalDigital': {'FechaTimbrado': datetime.datetime(2015, 2, 24, 11, 2, 43),
                                         'NoCertificadoSAT': '20001000000300003693',
                                         'SelloCFD': 'GJCjfU202yZovBi/xbQb3lFtd8sfqph35kANHIpBEHl4sfX86Xjj52KzeHHQhh0Mjn8VRerDh6QoFN2bzxulYFShoBgRN7x5MjjbIKjvXL5ln/8hdc1Gk1qj2NqJ0HrsV+ALr8BaCN7ZZZ/Zu7tqXNNxG3QOeqq4Gjwog48+Eio=',
                                         'SelloSAT': 'UMpaCt/83YSeXveeZjzbNwmMvA/PssjkIz/bFApzNvhcgp3UunKkH7IS0MxYqe65cbBJF4Ul9ASQsaBStVKPEhkP8yRPyeka/uKw4TCrMTxi2bKzj6Tcpw/GmkB4SDTDs+NsB/kO628kYv2cdd3AZcvu+TJi7CjcI6kx/aQQ+WI=',
                                         'UUID': 'a6d77767-7fa2-4acd-a8a0-f3126aff9fbe',
                                         'Version': '1.0'}},
 'CveRetenc': Code('14', 'Dividendos o utilidades distribuidas'),
 'DescRetenc': 'Dividendos',
 'Emisor': {'CURPE': 'AAAA010101HNLMNLD2',
            'NomDenRazSocE': 'Empresa DEMO para Rentenciones S de TST',
            'RFCEmisor': 'AAA010101AAA'},
 'FechaExp': datetime.datetime(2015, 2, 24, 11, 2, 43, tzinfo=datetime.timezone(datetime.timedelta(days=-1, seconds=64800))),
 'FolioInt': 'TEST00003',
 'NumCert': '20001000000100005867',
 'Periodo': {'Ejerc': 2014, 'MesFin': 6, 'MesIni': 6},
 'Receptor': {'Nacional': {'CURPR': 'VAHE820926HNLLRD02',
                           'NomDenRazSocR': 'PÚBLICO EN GENERAL S DE TST',
                           'RFCRecep': 'XAXX010101000'},
              'Nacionalidad': 'Nacional'},
 'Sello': 'GJCjfU202yZovBi/xbQb3lFtd8sfqph35kANHIpBEHl4sfX86Xjj52KzeHHQhh0Mjn8VRerDh6QoFN2bzxulYFShoBgRN7x5MjjbIKjvXL5ln/8hdc1Gk1qj2NqJ0HrsV+ALr8BaCN7ZZZ/Zu7tqXNNxG3QOeqq4Gjwog48+Eio=',
 'Totales': {'ImpRetenidos': [{'BaseRet': Decimal('1000.00'),
                               'Impuesto': Code('01', 'ISR'),
                               'MontoRet': Decimal('500.00'),
                               'TipoPagoRet': 'Pago definitivo'}],
             'MontoTotExent': Decimal('1000.00'),
             'MontoTotGrav': Decimal('13000.00'),
             'MontoTotOperacion': Decimal('14000.00'),
             'MontoTotRet': Decimal('3500.00')},
 'Version': '1.0'}